...
export default function SpareParts() {
  ...
  return (
    <div data-testid="spare-parts-page">
      ...
      {location && state && (
        <div className="max-w-[1200px] mx-auto px-6 pt-10 pb-4 flex flex-wrap items-center gap-2 text-xs text-zinc-400 relative z-10">
                    ...
                    <ul className="p-5 space-y-2">
                      {cat.items.map((item) => (
                        <li key={item} className="text-sm text-zinc-300 flex items-start gap-2">
                          <span className="text-zinc-200 shrink-0">▪</span> {item}
                        </li>
                      ))}
                    </ul>
                  </div>
      ...
      <section className="kg-section bg-surface-darker border-t border-zinc-800">
        <div className="max-w-[1200px] mx-auto text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            ...
            <Link href={`/service/power-weeder-repair-service-in-${citySlug}`} className="px-6 py-3 border border-zinc-800 rounded-md hover:border-lime-500 hover:text-lime-500 transition text-sm">
