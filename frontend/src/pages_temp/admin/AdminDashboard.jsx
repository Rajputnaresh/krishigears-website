...
export default function AdminDashboard() {
  ...
  if (!user) return <div className="min-h-screen grid place-items-center bg-black text-zinc-300">Loading…</div>;

  return (
    <div data-testid="admin-dashboard" className="min-h-screen bg-surface-darkest text-white">
      {/* Header */}
      ...
      <main className="max-w-[1400px] mx-auto px-6 py-10">
        ...
        <Tabs defaultValue="leads" className="w-full">
          <TabsList className="bg-surface-dark border border-zinc-800 flex-wrap h-auto">
            ...
            <TabsTrigger data-testid="tab-reviews" value="reviews">Reviews</TabsTrigger>
            <TabsTrigger data-testid="tab-blog" value="blog">Blog</TabsTrigger>
  ...
  return (
    <div>
      <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
            ...
            return (
              <button
                ...
                className={`px-4 py-2 border text-sm font-medium inline-flex items-center gap-2 transition ${
                  type === t.value ? "border-lime-500 text-lime-500 bg-lime-500/10" : "border-zinc-800 text-zinc-300 hover:border-zinc-600 hover:text-white"
                }`}
              >
        ...
        <button data-testid="leads-refresh" onClick={() => fetchLeads()} className="text-sm text-zinc-300 hover:text-lime-500 inline-flex items-center gap-2">
...
function LeadsList({ loading, leads, expanded, setExpanded, onRemove }) {
  ...
  if (leads.length === 0) return (
    <div className="text-center text-zinc-400 py-16 border border-dashed border-zinc-800 bg-background flex flex-col items-center justify-center">
          ...
          <div key={lead.id} className="border-b border-zinc-800 last:border-b-0">
            <div className="grid grid-cols-12 gap-4 p-4 items-center hover:bg-surface transition">
              ...
              <div className="col-span-1 flex justify-end gap-2">
                <button data-testid={`lead-expand-${lead.id}`} onClick={() => setExpanded(isOpen ? null : lead.id)} className="text-zinc-300 hover:text-lime-500">
                  {isOpen ? <ChevronUp className="h-4 w-4"/> : <ChevronDown className="h-4 w-4"/>}
                </button>
                <button data-testid={`lead-delete-${lead.id}`} onClick={() => onRemove(lead.id)} className="text-zinc-300 hover:text-red-500">
...
function BlogList({ loading, posts, onEdit, onDelete }) {
  if (loading) return <div className="text-center text-zinc-400 py-12">Loading…</div>;
  if (posts.length === 0) return <div className="text-center text-zinc-400 py-12 border border-dashed border-zinc-800">No blog posts yet. Click "New Post" to create your first.</div>;
  return (
    <div className="border border-zinc-800">
      {posts.map((p) => (
        <div key={p.slug} className="grid grid-cols-12 gap-4 p-4 items-center border-b border-zinc-800 last:border-b-0">
          <div className="col-span-1">
            {p.cover_image && <img src={p.cover_image} alt="" loading="lazy" className="h-12 w-12 object-cover border border-zinc-800"/>}
          </div>
          ...
          <div className="col-span-2 text-xs text-zinc-400">{new Date(p.created_at).toLocaleDateString()}</div>
          <div className="col-span-1 flex justify-end gap-2">
            <a href={`/blog/${p.slug}`} target="_blank" rel="noreferrer" className="text-zinc-300 hover:text-lime-500"><Eye className="h-4 w-4"/></a>
            <button onClick={() => onEdit(p)} data-testid={`blog-edit-${p.slug}`} className="text-zinc-300 hover:text-lime-500"><Pencil className="h-4 w-4"/></button>
            <button onClick={() => onDelete(p.slug)} data-testid={`blog-delete-${p.slug}`} className="text-zinc-300 hover:text-red-500"><Trash2 className="h-4 w-4"/></button>
          </div>
        </div>
      ))}
    </div>
...
function BlogEditor({ open, setOpen, post, onSaved }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="bg-background border-zinc-800 text-white max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
        ...
        <div className="space-y-4 mt-2">
          ...
          <div>
            <Label className="text-xs uppercase tracking-wider text-zinc-300">Slug (URL)</Label>
          ...
          <div>
            <Label className="text-xs uppercase tracking-wider text-zinc-300">Excerpt (short summary)</Label>
          ...
          <div>
            <Label className="text-xs uppercase tracking-wider text-zinc-300">Cover Image URL</Label>
          ...
          <div>
            <Label className="text-xs uppercase tracking-wider text-zinc-300">Tags (comma separated)</Label>
          ...
          <div>
            <Label className="text-xs uppercase tracking-wider text-zinc-300">Content (Markdown supported: use ## for headings)</Label>
