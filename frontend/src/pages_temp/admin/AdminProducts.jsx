...
function ProductsList({ loading, filtered, onEdit, onDelete, onToggleActive, onToggleFeatured, onMove }) {
  if (loading) return <div className="text-center text-zinc-400 py-12">Loading products…</div>;
  if (filtered.length === 0) return <div className="text-center text-zinc-400 py-12 border border-dashed border-zinc-800">No products match your filters.</div>;
  return (
...
function ProductRow({ product, onEdit, onDelete, onToggleActive, onToggleFeatured, onMove }) {
  ...
  return (
    <div className="grid grid-cols-12 gap-3 p-3 items-center hover:bg-surface transition">
      ...
      <div className="col-span-2 flex items-center justify-end gap-2">
        ...
        <button onClick={onEdit} data-testid={`admin-edit-${product.slug}`} className="text-zinc-300 hover:text-lime-500"><Pencil className="h-4 w-4"/></button>
        <button onClick={onDelete} data-testid={`admin-delete-${product.slug}`} className="text-zinc-300 hover:text-red-500"><Trash2 className="h-4 w-4"/></button>
      </div>
    </div>
  );
}
...
function ProductEditor({ open, setOpen, product, onSaved }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="bg-background border-zinc-800 text-white max-w-4xl max-h-[92vh] overflow-y-auto">
        <DialogHeader>
            ...
            <div className="space-y-2">
              {form.specs.map((s, i) => (
                <div key={s.id} className="grid grid-cols-12 gap-2">
                  ...
                  />
                  <button onClick={() => removeArrItem("specs", i)} className="col-span-1 text-zinc-400 hover:text-red-500"><X className="h-4 w-4 mx-auto"/></button>
                </div>
              ))}
    ...
    <div className="space-y-2">
      {values.map((v, i) => (
        <div key={i} className="flex items-center gap-2">
          ...
          />
          <button onClick={() => onRemove(i)} className="text-zinc-400 hover:text-red-500 shrink-0"><X className="h-4 w-4"/></button>
        </div>
      ))}
