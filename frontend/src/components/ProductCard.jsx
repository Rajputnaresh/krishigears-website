  ...
  return (
    <div data-testid={`product-card-${product.slug}`} className="kg-card overflow-hidden flex flex-col relative group">
      ...
      {product.badges?.length > 0 && (
        <div className="absolute top-3 left-3 z-10 flex flex-wrap gap-1.5 max-w-[60%] pointer-events-none">
          {product.badges.slice(0, 2).map((b) => (
            <span key={b} className="text-[9px] font-bold tracking-wider uppercase px-2 py-1 bg-black/85 text-lime-400 border border-lime-500/40 backdrop-blur rounded">
              {b}
            </span>
          ))}
        </div>
      ...
      {/* WhatsApp quick-quote top-right float button (standalone clickable element) */}
      <a
        href={waHref}
        target="_blank"
        rel="noreferrer"
        onClick={() => trackWhatsAppClick("product_card", product.slug)}
        ...
        className="absolute top-3 right-3 z-20 h-12 w-12 min-h-[48px] min-w-[48px] grid place-items-center bg-whatsapp hover:bg-whatsapp-hover text-white rounded-full shadow-xl hover:scale-110 active:scale-95 transition"
      >
        <WhatsAppIcon className="h-5 w-5" />
      </a>

      {/* Primary Product Card Link (navigates to PDP) */}
      <Link
        href={`/products/${product.slug}`}
        className="flex flex-col flex-1 focus:outline-none focus:ring-2 focus:ring-lime-500 rounded-t-lg"
      >
        <div className="aspect-[4/3] bg-zinc-900/40 border-b border-zinc-800/80 overflow-hidden relative group-hover:bg-zinc-850/60 transition-colors">
          <div className="w-full h-full p-3 flex items-center justify-center">
            <div className="w-full h-full rounded-md bg-zinc-950/70 p-2 flex items-center justify-center border border-zinc-800/40">
              <Image
        ...
        <div className="p-5 flex-1 flex flex-col justify-between">
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-y-2 gap-x-4 border-y border-zinc-800/60 py-2.5 my-1">
      ...
      <div className="px-5 pb-5 pt-3 border-t border-zinc-800/80 flex items-center justify-between gap-2 bg-zinc-950/40">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            href={`/products/${product.slug}`}
            className="h-9 w-9 min-h-[36px] min-w-[36px] grid place-items-center rounded bg-zinc-900 border border-zinc-700 text-lime-400 hover:text-white hover:border-lime-500 transition active:scale-95"
