...
export default function EnquiryDialog({ trigger, product = "" }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="bg-background border-zinc-800 text-white max-w-lg">
        <DialogHeader>
        ...
        <form onSubmit={submit} className="space-y-4 mt-2">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label className="text-xs uppercase tracking-wider text-zinc-300">Name*</Label>
              <Input data-testid="enquiry-name" value={form.name} onChange={update("name")} className="bg-black border-zinc-800 mt-1.5" />
            </div>
            <div>
              <Label className="text-xs uppercase tracking-wider text-zinc-300">Phone*</Label>
              <Input data-testid="enquiry-phone" value={form.phone} onChange={update("phone")} className="bg-black border-zinc-800 mt-1.5" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label className="text-xs uppercase tracking-wider text-zinc-300">Email</Label>
          ...
          {!product && (
            <div>
              <Label className="text-xs uppercase tracking-wider text-zinc-300">Product of Interest</Label>
              <Input data-testid="enquiry-product" value={form.product} onChange={update("product")} className="bg-black border-zinc-800 mt-1.5" />
            </div>
          )}
          <div>
            <Label className="text-xs uppercase tracking-wider text-zinc-300">Message</Label>
