        ...
        <div className="max-w-[1400px] mx-auto relative grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <div className="mt-8 flex gap-3">
              <Link to="/become-a-dealer" data-testid="dn-become-dealer" className="bg-lime-500 hover:bg-lime-400 text-black font-bold px-7 py-4 rounded-md inline-flex items-center gap-2">
                Join Network <ArrowRight className="h-4 w-4" />
              </Link>
              <Link to="/contact" data-testid="dn-contact" className="border border-zinc-700 hover:border-lime-500 hover:text-lime-500 px-7 py-4 font-bold rounded-md">Find Local Dealer</Link>
            </div>
          </div>
            ...
            {REGIONS.map((r) => (
              <div key={r.name} className="kg-card p-7">
                ...
                <div className="flex flex-wrap gap-2 mt-4">
                  {r.states.map((s) => (
                    <span key={s} className="text-xs px-3 py-1 border border-zinc-800 text-zinc-400">{s}</span>
                  ))}
                </div>
              </div>
            ))}
