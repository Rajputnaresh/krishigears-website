...
const file = '../backend/seed_blog.py';
...
// It's a bit tricky to safely regex multiline python dicts with JS, I will just do simple replaces for the specific slugs

const pyEscape = str => '"""\\\n' + str.replace(/\\/g, '\\\\') + '\n"""';

content = content.replace(/("slug": "power-weeder-buying-guide-2026".*?"content": )"Content coming soon..."/s, `$1${pyEscape(buying_guide)}`);
content = content.replace(/("slug": "rk-170f-vs-177f-vs-173f-comparison".*?"content": )"Content coming soon..."/s, `$1${pyEscape(comparison)}`);
content = content.replace(/("slug": "power-weeder-government-subsidy-dbt-guide".*?"content": )"Content coming soon..."/s, `$1${pyEscape(subsidy)}`);

fs.writeFileSync(file, content);
console.log("Updated seed_blog.py");
